...
async def insert_reviews_by_id(book_id: int,
                               body: ReviewSchema,
...
async def get_summary(book_id: int, db: AsyncSession) -> SUMMARY_TYPE:
    ...
    result = await db.execute(select(func.avg(Reviews.rating)).where(Reviews.book_id
                                                                     == book_id))
    avg_rating = result.scalar()


    result = await db.execute(select(Reviews).where(Reviews.book_id ==
                                                    book_id).order_by(desc(
        Reviews.created_at)).limit(LATEST_REVIEW_LIMIT))
